...
bool OutNtupleProc::OpenFile(std::string filename) {
  ...
  if (options.tracking) {
    ...
    outputTree->Branch("trackQuenched", &trackQuenched);
...
Processor::Result OutNtupleProc::DSEvent(DS::Root *ds) {
  ...
  if (options.tracking) {
    ...
    std::vector<double> kinetic, globaltime, quenched;
    ...
    for (int trk = 0; trk < nTracks; trk++) {
      ...
      for (int stp = 0; stp < nSteps; stp++) {
        ...
        quenched.push_back(step->GetScintEdepQuenched());
      ...
      trackQuenched.push_back(quenched);
