...
bool OutNtupleProc::OpenFile(std::string filename) {
  ...
  std::vector<std::string> my_volumes = {"sodium_iodide", "scintillator"};
...
Processor::Result OutNtupleProc::DSEvent(DS::Root* ds) {
  ...
  trackProcess.clear();
  trackVolume.clear();
  std::vector<std::string> check_volumes = {"sodium_iodide", "scintillator"};
  ...
  for (int trk = 0; trk < nTracks; trk++) {
    ...
    for (int stp = 0; stp < nSteps; stp++) {
      ...
      xtrack.push_back(tv.X());  // store as mm
      ...
      if ((vol == "sodium_iodide") || vol == ("scintillator")) {
