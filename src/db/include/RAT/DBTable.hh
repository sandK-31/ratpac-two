...
class DBTable {
 public:
  ...
  /** Get a vector of field names.  Call GetFieldType() to determine the type of
   * each.*/
...
/** Exception: Field has wrong type in RATDB.
    Inherits from DBNotFoundError to work with existing code that only expects
   that exception.
 */
class DBWrongTypeError : public DBNotFoundError {
 public:
   ...
   * requested type
   *  @param _index  Index of table in which field could not be found with
   * requested type
